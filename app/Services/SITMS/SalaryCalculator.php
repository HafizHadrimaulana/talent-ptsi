...
class SalaryCalculator
{
     ...
     */
    public function calculate(float $gajiPokok, string $startDate, string $endDate, float $thr, float $kompensasi, string $riskLevel = 'Rendah'): array
    {
        ...
        $penghasilanBrutoSebulan = $gajiPokok + $bpjsKetenagakerjaan + $jp + $bpjsKesehatan;
        $thpSetahun = $penghasilanBrutoSebulan * 12; 
        $ptkp = 54000000; 
        $pkp = $thpSetahun - $ptkp;
        
        // Pembulatan PKP ke ribuan ke bawah (Floor to nearest thousand) - Aturan Pajak Umum
        ...
        if ($pkp <= 0) {
            ...
            if ($pkp <= 60000000) {
                $pajakPertahun = $pkp * 0.05;
            } 
            elseif ($pkp > 60000000 && $pkp <= 250000000) {
                $pajakPertahun = (60000000 * 0.05) + (($pkp - 60000000) * 0.15);
            } 
            elseif ($pkp > 250000000 && $pkp <= 500000000) {
                $pajakPertahun = (60000000 * 0.15) + (200000000 * 0.15) + (($pkp - 250000000) * 0.25);
            } 
            else {
                $pajakPertahun = (60000000 * 0.05) + (200000000 * 0.15) + (250000000 * 0.25) + (($pkp - 500000000) * 0.30);
            }
        }
        $pajakPerBulan = $pajakPertahun / 12;
