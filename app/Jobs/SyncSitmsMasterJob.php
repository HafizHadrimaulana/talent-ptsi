...
class SyncSitmsMasterJob implements ShouldQueue
{
    ...
    public function handle(HttpSitmsClient $client): void
    {
        ...
        if ($this->dryRun && $this->rawExportPath) {
            ...
            if ($csv)
                fputcsv($csv, ['external_id', 'full_name', 'unit', 'position', 'email']);
        }
        ...
        do {
            ...
            foreach ($rows as $row) {
                ...
                if ($externalId)
                    $this->seenExternalIds[] = (string) $externalId;
                if ($employeeId)
                    $this->seenEmployeeIds[] = (string) $employeeId;

                if ($this->dryRun) {
                    [$extId, $fullName, $unit, $pos, $email] = $this->rowFingerprint($row);
                    $this->seenExternalIds[] = (string) ($extId ?? uniqid('raw-', true));
                    if ($csv)
                        fputcsv($csv, [(string) ($extId ?? ''), $fullName, $unit, $pos, $email]);
        ...
        if (!$this->dryRun) {
            if (!empty($this->seenJobHistoryIds))
                $this->mirrorJobHistoriesHard();
            if (!empty($this->seenEducationIds))
                $this->mirrorEducationsHard();
            if (!empty($this->seenTrainingIds))
                $this->mirrorTrainingsHard();
            if (!empty($this->seenCertificationIds))
                $this->mirrorCertificationsHard();
            if (!empty($this->seenDocumentIds))
                $this->mirrorDocumentsHard();
            if (!empty($this->seenPortfolioIds))
    ...
    {
        if (!Schema::hasTable('employees') || empty($seenIds))
            return;
    ...
    {
        if (!Schema::hasTable($table) || empty($seenMap))
            return;
    ...
    protected function upsertEmployeeRawFromSitmsRow(array $row): bool
    {
        ...
        if (in_array('employee_id', $colsEmp, true))
            $payload['employee_id'] = $this->cut($employeeIdFinal);
        if (in_array('id_sitms', $colsEmp, true) && $sitmsId !== null)
            $payload['id_sitms'] = $this->cut($sitmsId);
        if (in_array('is_active', $colsEmp, true))
            $payload['is_active'] = isset($row['is_active']) ? ((int) $row['is_active'] ? 1 : 0) : 1;
        if (in_array('directorate_id', $colsEmp, true))
            $payload['directorate_id'] = $directorateId;
        if (in_array('unit_id', $colsEmp, true))
            $payload['unit_id'] = $unitId;
        if (in_array('position_id', $colsEmp, true))
            $payload['position_id'] = $positionId;
        if (in_array('position_level_id', $colsEmp, true))
            $payload['position_level_id'] = $positionLevelId;
        if (in_array('location_id', $colsEmp, true))
            $payload['location_id'] = $locationId;
        if (in_array('home_base_raw', $colsEmp, true))
            $payload['home_base_raw'] = isset($row['home_base']) ? $this->cut(strip_tags((string) $row['home_base']), 800) : null;
        if (in_array('home_base_city', $colsEmp, true))
            $payload['home_base_city'] = $this->cut($hbCity);
        if (in_array('home_base_province', $colsEmp, true))
            $payload['home_base_province'] = $this->cut($hbProv);
        ...
        if (Schema::hasTable('employees_snapshot')) {
            ...
            if ($snapshotKey) {
                ...
                if ($snapshotKeyValue) {
                    ...
                    if (in_array('created_at', $colsSnap, true))
        ...
        foreach (Arr::get($row, 'education_list.education_data', []) as $e)
            $this->upsertEducation($personId, (array) $e);
        foreach (Arr::get($row, 'training_list.training_data', []) as $t)
            $this->upsertTraining($personId, (array) $t);
        foreach (Arr::get($row, 'brevet_list.brevet_data', []) as $c)
            $this->upsertCertification($personId, (array) $c);
        foreach (Arr::get($row, 'jobs_list.jobs_data', []) as $j)
            $this->upsertJobHistory($personId, (array) $j);
    ...
    protected function ensurePersonIdForRow(array $row, ?string $existingPersonId = null): string
    {
        ...
        if ($existingPersonId) {
            $upd = [];
            if (in_array('full_name', $cols, true) && !empty($row['full_name']))
                $upd['full_name'] = $this->cut($row['full_name']);
            if (in_array('gender', $cols, true) && isset($row['gender']))
                $upd['gender'] = $this->cut($row['gender']);
            if (in_array('date_of_birth', $cols, true))
                $upd['date_of_birth'] = $this->parseDate($row['date_of_birth'] ?? null);
            if (in_array('place_of_birth', $cols, true) && isset($row['place_of_birth']))
                $upd['place_of_birth'] = $this->cut($row['place_of_birth']);
            if (in_array('phone', $cols, true) && isset($row['contact_no']))
                $upd['phone'] = $this->cut($row['contact_no'], 50);
            if (in_array('email', $cols, true) && isset($row['email']))
                $upd['email'] = $this->cut($row['email'], 150);
            if (in_array('address', $cols, true) && isset($row['address']))
                $upd['address'] = $this->cut($row['address']);
            if (in_array('city', $cols, true) && isset($row['city']))
                $upd['city'] = $this->cut($row['city'], 120);
            if (in_array('nik_hash', $cols, true) && !empty($rawNik))
                $upd['nik_hash'] = $rawNik;
            if (in_array('nik_last4', $cols, true))
                $upd['nik_last4'] = $nikLast4;
        ...
        if (in_array('full_name', $cols, true))
            $person['full_name'] = $this->cut($this->nullIfEmpty($row['full_name'] ?? null)) ?? '-';
        if (in_array('gender', $cols, true))
            $person['gender'] = $this->cut($this->nullIfEmpty($row['gender'] ?? null));
        if (in_array('date_of_birth', $cols, true))
            $person['date_of_birth'] = $this->parseDate($row['date_of_birth'] ?? null);
        if (in_array('place_of_birth', $cols, true))
            $person['place_of_birth'] = $this->cut($this->nullIfEmpty($row['place_of_birth'] ?? null));
        if (in_array('phone', $cols, true))
            $person['phone'] = $this->cut($this->nullIfEmpty($row['contact_no'] ?? null), 50);
        if (in_array('email', $cols, true))
            $person['email'] = $this->cut($this->nullIfEmpty($row['email'] ?? null), 150);
        if (in_array('address', $cols, true))
            $person['address'] = $this->cut($this->nullIfEmpty($row['address'] ?? null));
        if (in_array('city', $cols, true))
            $person['city'] = $this->cut($this->nullIfEmpty($row['city'] ?? null), 120);
        if (in_array('nik_hash', $cols, true) && !empty($rawNik))
            $person['nik_hash'] = $rawNik;
        if (in_array('nik_last4', $cols, true))
            $person['nik_last4'] = $nikLast4;
    ...
    {
        if (isset($this->tableColumnsMeta[$table]))
            return $this->tableColumnsMeta[$table];
        $meta = [];
        ...
        try {
            ...
            foreach ($cols as $c)
                $meta[$c->Field] = ['type' => $c->Type, 'null' => strtoupper((string) $c->Null) === 'YES', 'default' => $c->Default];
        } catch (\Throwable $e) {
        }
    ...
    protected function sitmsInsertPortfolio(string $personId, string $category, array $data): void
    {
        ...
        $row = [
            'person_id' => $personId,
            'category' => $category,
            'title' => $this->cut($data['title'] ?? null, 150),
            'organization' => $this->cut($data['organization'] ?? null, 150),
            'start_date' => $data['start_date'] ?? null,
            'end_date' => $data['end_date'] ?? null,
            'description' => $this->cut($data['description'] ?? null, 300),
            'meta' => isset($data['meta']) ? json_encode($data['meta']) : null,
            'created_at' => now(),
            'updated_at' => now()
        ];
        if (in_array('source_system', $cols, true))
            $row['source_system'] = 'sitms';
        ...
        if ($id) {
            if (!isset($this->seenPortfolioIds[$personId]))
                $this->seenPortfolioIds[$personId] = [];
    ...
    protected function sitmsSyncDocuments(string $personId, array $row): void
    {
        ...
        foreach (Arr::get($row, 'documents_list.documents_data', []) as $d) {
            ...
            if (in_array('doc_type', $cols, true))
                $payload['doc_type'] = $this->cut($this->nullIfEmpty($d['document_type'] ?? null) ?? 'unknown', 50);
            ...
            if ($id) {
                if (!isset($this->seenDocumentIds[$personId]))
                    $this->seenDocumentIds[$personId] = [];
    ...
    protected function upsertEducation(string $personId, array $e): void
    {
        ...
        if (in_array('source_system', $this->tableColumns('educations'), true))
            $row['source_system'] = 'sitms';
        ...
        if ($id) {
            if (!isset($this->seenEducationIds[$personId]))
                $this->seenEducationIds[$personId] = [];
    ...
    protected function upsertTraining(string $personId, array $t): void
    {
        ...
        if (in_array('source_system', $this->tableColumns('trainings'), true))
            $row['source_system'] = 'sitms';
        ...
        if ($id) {
            if (!isset($this->seenTrainingIds[$personId]))
                $this->seenTrainingIds[$personId] = [];
    ...
    protected function upsertCertification(string $personId, array $c): void
    {
        ...
        if (in_array('source_system', $this->tableColumns('certifications'), true))
            $row['source_system'] = 'sitms';
        ...
        if ($id) {
            if (!isset($this->seenCertificationIds[$personId]))
                $this->seenCertificationIds[$personId] = [];
    ...
    {
        if (!Schema::hasTable('job_histories'))
            return;
        ...
        if (in_array('source_system', $this->tableColumns('job_histories'), true))
            $row['source_system'] = 'sitms';
        ...
        if ($id) {
            if (!isset($this->seenJobHistoryIds[$personId]))
                $this->seenJobHistoryIds[$personId] = [];
    ...
    protected function sitmsEnsureLookupId(string $table, ?string $name, array $extras = []): ?int
    {
        ...
        if ($table === 'units') {
            ...
            if (in_array('name', $cols, true))
                $row['name'] = $this->cut($name);
            if (in_array('code', $cols, true))
                $row['code'] = $wantCode ?: $this->ensureUniqueCode($table, Str::upper(Str::substr(preg_replace('/[^a-zA-Z0-9]/', '', $name), 0, 10)));
            if ($dirId && in_array('directorate_id', $cols, true))
                $row['directorate_id'] = $dirId;
            if (in_array('category', $cols, true))
                $row['category'] = null; // Default to null for new auto-synced units
        ...
        if (in_array('name', $cols, true))
            $row['name'] = $this->cut($name);
        if (in_array('code', $cols, true))
            $row['code'] = $this->ensureUniqueCode($table, Str::upper(Str::substr(preg_replace('/[^a-zA-Z0-9]/', '', $name), 0, 10)));
    ...
    protected function sitmsParseHomeBase(?string $homeBase): array
    {
        ...
        $city = null;
        $prov = null;
        if (preg_match('~<i>(.*?)</i>~u', $homeBase, $m))
            $city = $this->nullIfEmpty($m[1] ?? null);
