<aside class="sidebar glass" id="sidebar" aria-label="Primary navigation" data-scroll-area> 
    ...
    if ($isAdminSystem) {
        $targetDashboardRoute = route('admin.dashboard');
    } elseif ($isPelamar) {
        $targetDashboardRoute = route('employee.dashboard');
    }
    ...
    $isDashActive = request()->routeIs('employee.dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('dashboard');
    ...
    $isHumanCapital = $user && strtolower($user->employee?->unit?->name ?? '') === 'divisi human capital';
    @endphp

    <div class="brand">
        <a href="{{ $targetDashboardRoute }}" class="brand-link" aria-label="Dashboard">
    ...
    <nav class="nav-section">
        ...
        <div class="nav">
            <a class="nav-item {{ $isDashActive ? 'active' : '' }}" href="{{ $targetDashboardRoute }}">
    ...
    <nav class="nav-section">
        ...
        <div class="nav">
            <button type="button" class="nav-item js-accordion {{ $recOpen ? 'open' : '' }} {{ $isRecActive ? 'active' : '' }}"
                ...
                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
            ...
            <div id="nav-recruitment" class="nav-children {{ $recOpen ? 'open' : '' }}" data-accordion-panel="nav-recruitment">
                ...
                <a class="nav-item nav-child {{ request()->routeIs('recruitment.applicant-data.*') ? 'active' : '' }}" href="{{ route('recruitment.applicant-data.index') }}">
                    <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
                ...
                <a class="nav-item nav-child {{ request()->routeIs('recruitment.contracts*')?'active':'' }}" href="{{ route('recruitment.contracts.index') }}">
    ...
    <nav class="nav-section">
        ...
        <div class="nav">
            <button type="button" class="nav-item js-accordion {{ $trOpen ? 'open' : '' }} {{ $isTrActive ? 'active' : '' }}"
                ...
                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z" /><path d="M6 12v5c3 3 9 3 12 0v-5" /></svg>
            ...
            <div id="nav-training" class="nav-children {{ $trOpen ? 'open' : '' }}" data-accordion-panel="nav-training">
                ...
                <a class="nav-item nav-child {{ request()->routeIs('training.dashboard')?'active':'' }}" href="{{ route('training.dashboard') }}">
                    <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18" /><path d="m19 9-5 5-4-4-3 3" /></svg>
                ...
                @if(
                    $isSuper ||
                    (
                        $user?->can('training.management.view') &&
                        (
                            $user->hasRole('DHC') ||
                            $user->hasRole('SDM Unit') ||
                            $user->hasRole('Kepala Unit') ||
                            ($user->hasRole('AVP') && $isHumanCapital)
                        )
                    )
                )
    ...
    <nav class="nav-section">
        ...
        <div class="nav">
            ...
            <div id="nav-access" class="nav-children {{ $acOpen ? 'open' : '' }}" data-accordion-panel="nav-access">
                ...
                <a class="nav-item nav-child {{ request()->routeIs('admin.permissions.*') ? 'active' : '' }}" href="{{ route('admin.permissions.index') }}">
    ...
    <nav class="nav-section">
        ...
        <div class="nav">
            <button type="button" class="nav-item js-accordion {{ $mdOpen ? 'open' : '' }} {{ $isMasterActive ? 'active' : '' }}" data-accordion="nav-masterdata" aria-expanded="{{ $mdOpen ? 'true' : 'false' }}">
                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" /><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" /></svg>
