    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="u-space-y-sm">
                ...
                <input type="text" name="work_list[{{$idx}}][company]" value="{{ $work['company'] ?? '' }}" 
                       class="u-input w-full" placeholder="PT Contoh Sejahtera">
            </div>
            <div class="u-space-y-sm">
                <label class="u-block u-text-xs u-font-medium u-mb-sm text-gray-500 uppercase">Jabatan / Posisi</label>
                <input type="text" name="work_list[{{$idx}}][position]" value="{{ $work['position'] ?? '' }}" 
                       class="u-input w-full" placeholder="Staff IT">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="u-space-y-sm">
                ...
                <input type="text" name="work_list[{{$idx}}][city]" value="{{ $work['city'] ?? '' }}" 
                       class="u-input w-full" placeholder="Jakarta, Indonesia">
            </div>
            <div class="u-space-y-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="u-space-y-sm">
                ...
                <input type="number" name="work_list[{{$idx}}][salary]" value="{{ $work['salary'] ?? '' }}" 
                       class="u-input w-full" placeholder="0">
            </div>
            <div class="u-space-y-sm">
                <label class="u-block u-text-xs u-font-medium u-mb-sm text-gray-500 uppercase">Tahun Mulai</label>
                <input type="number" name="work_list[{{$idx}}][start_year]" value="{{ $work['start_year'] ?? '' }}" 
            ...
            <div class="u-space-y-sm">
                ...
                <input type="number" name="work_list[{{$idx}}][end_year]" value="{{ $work['end_year'] ?? '' }}" 
                       class="u-input w-full" placeholder="YYYY (Isi 'Sekarang' jika aktif)">
            </div>
        </div>
        ...
        <div class="u-space-y-sm">
            ...
            <input type="text" name="work_list[{{$idx}}][reason]" value="{{ $work['reason'] ?? '' }}" 
                   class="u-input w-full" placeholder="Kontrak habis / Resign / dll">
        </div>

        {{-- Deskripsi --}}
        <div class="u-space-y-sm">
            <label class="u-block u-text-xs u-font-medium u-mb-sm text-gray-500 uppercase">Deskripsi Pekerjaan</label>
            <textarea name="work_list[{{$idx}}][desc]" class="u-input w-full h-24 py-2" 
                      placeholder="Jelaskan tanggung jawab dan pencapaian Anda...">{{ $work['desc'] ?? '' }}</textarea>
        </div>
    </div>
</div>