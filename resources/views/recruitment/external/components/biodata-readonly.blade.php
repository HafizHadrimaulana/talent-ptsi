                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($person->education_history ?? [] as $edu)
                            <tr>
                                <td class="px-6 py-4 font-bold">{{ $edu['level'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $edu['name'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $edu['major'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $edu['year'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $edu['gpa'] ?? '-' }}</td>
                            </tr>
                        @empty
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($person->family_data ?? [] as $fam)
                            <tr>
                                <td class="px-6 py-4 font-bold">{{ $fam['relation'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $fam['name'] ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $fam['job'] ?? '-' }}</td>
                            </tr>
                        @empty
                    ...
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <div class="text-xs font-bold bg-gray-100 px-2 py-1 rounded text-gray-600">
                                {{ $work['start_year'] ?? '' }} - {{ $work['end_year'] ?? 'Sekarang' }}
                            </div>
                        </div>
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <ul class="list-disc pl-5 text-sm space-y-1 text-gray-700">
                        @forelse($person->organization_experience ?? [] as $org)
                            <li>
                                <b>{{ $org['name'] ?? '-' }}</b> | {{ $org['position'] ?? '-' }} ({{ $org['start_year'] ?? '-' }} - {{ $org['end_year'] ?? '-' }})
                                <br>
                                {{ $org['desc'] ?? '' }}
                            </li>
                        @empty
